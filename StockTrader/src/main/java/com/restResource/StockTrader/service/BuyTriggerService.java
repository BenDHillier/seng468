            ...
            public void run(){
                for (;;) {
                    ...
                    if (!buyStockSnapshot.isPresent()) { //we dont have an entry yet, so continue waiting
                        ...
                        if(accountRepository.removeFunds(userId, amount*stockCost) == 0){
                            buyTriggerRepository.delete(buyStockSnapshot.get()); //remove it since it is not valid
                            //TODO might want an error message since nothing tells the user it failed
                            return;
                        }
                ...
                //TODO technically can assign this originally
                Integer cost;
                Integer amount;
                for (;;) {
                    ...
                    if (quote.getPrice() <= cost) {
                        ...
                        if(refund > 0){
                            ...
                            accountRepository.updateAccountBalance(userId, refund);
                        }
                        buyTriggerRepository.delete(buyStockSnapshot.get());
                        return;
                    }
                    try{
                        Thread.sleep(60000); //we sleep for 60 seconds every time, since we make the buy decision very quickly
                    }catch(InterruptedException e){
                        throw new IllegalArgumentException(
